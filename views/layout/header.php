<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <meta http-equiv="x-ua-compatible" content="ie=edge">
    <link rel="icon" sizes="192x192" href="assets/img/logo.png">
    <title>Sigma</title>
    <!-- Font Awesome Icons -->
    <link rel="stylesheet" href="assets/plugins/fontawesome-free/css/all.min.css">
    <!-- Theme style -->
    <link rel="stylesheet" href="assets/css/adminlte.min.css">
    <!-- Google Font: Source Sans Pro -->
    <link href="https://fonts.googleapis.com/css?family=Source+Sans+Pro:300,400,400i,700" rel="stylesheet">
    <!-- overlayScrollbars -->
    <link rel="stylesheet" href="assets/plugins/overlayScrollbars/css/OverlayScrollbars.min.css">
    <!-- Toastr -->
    <link rel="stylesheet" href="assets/plugins/toastr/toastr.min.css">
    <!-- Header -->
    <link rel="stylesheet" href="assets/css/header.css?v=1">
    <!-- REQUIRED SCRIPTS -->
    <noscript>This Site requires JavaScript! Este sitio require JavaScript!
        <style>
        .wrapper {display:none;}
        </style>
    </noscript>
    <!-- jQuery -->
    <script src="https://adminlte.io/themes/v3/plugins/jquery/jquery.min.js"></script>
    <!-- Bootstrap 4 -->
    <script src="https://adminlte.io/themes/v3/plugins/bootstrap/js/bootstrap.bundle.min.js"></script>
    <!-- AdminLTE App -->
    <script src="https://adminlte.io/themes/v3/dist/js/adminlte.min.js"></script>
    <!-- overlayScrollbars -->
    <script src="assets/plugins/overlayScrollbars/js/jquery.overlayScrollbars.min.js"></script>
    <script src="assets/plugins/sweetalert2/sweetalert2.all.min.js"></script>
    <script src="assets/plugins/toastr/toastr.min.js"></script>
    <script>
        setInterval(function(){$.post('?c=Init&a=SessionRefresh');},600000); //refreshes the session every 10 minutes
    </script>
</head>

<body class="hold-transition sidebar-mini sidebar-collapse text-sm layout-fixed layout-navbar-fixed">

    <!-- Modals -->
    <div class="modal fade" id="xlModal" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-xl" role="document">
            <div class="modal-content">
            </div>
        </div>
    </div>

    <div class="modal fade" id="lgModal" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
            </div>
        </div>
    </div>

    <div class="modal fade" id="defModal" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
            </div>
        </div>
    </div>

    <div class="modal fade" id="xsModal" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-sm" role="document">
            <div class="modal-content">
            </div>
        </div>
    </div>

    <div id="loading"></div>

    <div class="wrapper">
    
        <!-- Navbar -->
        <nav class="main-header navbar navbar-expand navbar-dark text-sm">
            <!-- Left navbar links -->
            <ul class="navbar-nav">
                <li class="nav-item">
                    <a class="nav-link" data-widget="pushmenu" href="#"><i class="fas fa-bars"></i></a>
                </li>
            </ul>


            <!-- Right navbar links -->

            <ul class="navbar-nav ml-auto">
                <!-- Notifications Dropdown Menu -->
                
                <li class="nav-item dropdown" id="task-dropdown">
                    <a class="nav-link" data-toggle="dropdown" href="#" aria-expanded="false" id="task-counter">
                        <i class="far fa-bell"></i>
                    </a>
                    <div class="dropdown-menu dropdown-menu-lg dropdown-menu-right" id="task-alerts" style="max-height:500px;overflow:auto"></div>
                </li>
                

      
            <!-- User Dropdown Menu -->
        <li class="nav-item dropdown">
            <a class="nav-link" data-toggle="dropdown" href="#" aria-expanded="false">
            <i class="far fa-user"></i>
            </a>
            <div class="dropdown-menu dropdown-menu-lg dropdown-menu-right">
            <a href="?c=Users&a=Profile" class="dropdown-item">
                <!-- Message Start -->
                <div class="media">
                <i class="far fa-user fa-3x mr-3"></i>
                <div class="media-body">
                    <h3 class="dropdown-item-title">
                    <?php echo $user->username ?>
                    </h3>
                    <p class="text-sm text-muted"><i class="far fa-clock mr-1"></i> <?php echo $user->createdAt ?></p>
                </div>
                </div>
            </a>
            <div class="dropdown-divider"></div>
            <a href="?c=Login&a=Logout" class="dropdown-item dropdown-footer float-rigt"><i class="fa fa-times"></i> Logout</a>
            </div>

            </ul>
        </nav>
        <!-- /.navbar -->

        <!-- Main Sidebar Container -->
        <aside class="main-sidebar sidebar-dark-primary elevation-4">
            <!-- Brand Logo -->
            <a href="?c=Init&a=Index" class="brand-link">
                <img src="assets/img/logo.png" style="width:30px" class="brand-image elevation-3">
                <span class="brand-text font-weight-light pl-3">Sigma</span>
            </a>

            <!-- Sidebar -->
            <div class="sidebar">

                <!-- Sidebar Menu -->
                <nav class="mt-2">
                    <ul class="nav nav-pills nav-sidebar flex-column nav-child-indent text-sm" data-widget="treeview" role="menu" data-accordion="false">
                    <?php 
                    $permissionsTitle = array();
                    $permissions = json_decode($this->users->permissionsGet($_SESSION["id-SIGMA"])->permissions, true);
                    foreach($permissions as $p) { 
                        $permissionsTitle[] = $this->init->permission($p)->title;
                    };
                    foreach($this->init->navTitleList() as $t) { 
                    if (in_array($t->title, $permissionsTitle)) {
                    ?>
                        <li class="nav-item has-treeview">
                            <a href="#" class="nav-link <?php echo ($t->c == $_REQUEST['c']) ? 'active' : '' ?>">
                                <?php echo $t->icon ?>
                                <p>
                                    <?php echo isset($lang[$t->title]) ? $lang[$t->title] : $t->title?>
                                    <i class="right fas fa-angle-left"></i>
                                </p>
                            </a>
                            <ul class="nav nav-treeview" style="display: none;">
                            
                            <?php foreach($this->init->navSubtitleList($t->title) as $s) {
                            if (in_array($s->id, $permissions)) { ?>
                                <li class="nav-item">
                                    <a href="?c=<?php echo $s->c ?>&a=<?php echo $s->a ?>" <?php echo $s->attributes ?>
                                        class="nav-link <?php echo ($s->c == $_REQUEST['c'] and $s->a == $_REQUEST['a']) ? 'active' : '' ?>">
                                        <i class="far fa-circle nav-icon"></i>
                                        <p><?php echo isset($lang[$s->subtitle]) ? $lang[$s->subtitle] : $s->subtitle ?></p>
                                    </a>
                                </li>
                            <?php }} ?>
                            </ul>
                        </li>
                        <?php }} ?>
                    </ul>
                </nav>
                <!-- /.sidebar-menu -->
            </div>
            <!-- /.sidebar -->
        </aside>

        <!-- Content Wrapper. Contains page content -->

        <div class="content-wrapper">

<script>
$(document).ready(function(){
    alerts();
    setInterval(function(){ 
        alerts();; 
 }, 5000);
});    


function alerts() {
    $.post('?c=Init&a=Alerts', { 'id': true },function(data) {
        if(data.trim() != ''){
        $("#task-counter").html(`<i class="far fa-bell"></i><span class="badge bg-danger navbar-badge alert">${data}</span>`);
        } else {
            $("#task-counter").html(`<i class="far fa-bell"></i>`);
        }
    }); 
}

$('#task-dropdown').on('show.bs.dropdown', function () {
    $.post("?c=Init&a=Alerts", function(data) {
        $("#task-alerts").html( data );
    });
});

$(document).on("click", ".purchaseAction", function() {
    $("#loading").show();
    id = $(this).data('id');
    status = $(this).data('status');
    title = $(this).data('title');
    $.post("?c=Purchases&a=Purchase",{id,status,title}).done(function(data){
        $("#loading").hide();
        $('#xlModal').modal('show');
        $('#xlModal .modal-content').html(data);
        $('.select2').select2(
            {
            tags:true,
        }
        );
    });
});

$(document).on("click", ".alertReceiveCheck", function() {
    $("#loading").show();
    id = $(this).data('id');
    status = 'view';
    title = 'View Purchase';
    $.post( "?c=Purchases&a=AlertCheck", { id }).done(function( data ) {
        $("#loading").hide();
        $.post( "?c=Purchases&a=Purchase", { id,status,title }).done(function( data ) {
            $('#xlModal').modal('toggle');
            $('#xlModal .modal-content').html(data);
        });
    });
});

$(document).on("click", ".alertReceiveCheckItem", function() {
    $("#loading").show();
    id = $(this).data('id');
    purchase = $(this).data('purchase');
    status = 'view';
    title = 'View Purchase';
    $.post( "?c=Purchases&a=AlertCheckItem", { id }).done(function( data ) {
        $("#loading").hide();
        $.post( "?c=Purchases&a=Purchase", { id: purchase,status,title }).done(function( data ) {
            $('#xlModal').modal('toggle');
            $('#xlModal .modal-content').html(data);
        });
    });
});

$(document).on("click", ".alertReceiveCheckItemPM", function() {
    $("#loading").show();
    id = $(this).data('id');
    purchase = $(this).data('purchase');
    status = 'view';
    title = 'View Purchase';
    $.post( "?c=Purchases&a=AlertCheckItemPM", { id }).done(function( data ) {
        $("#loading").hide();
        $.post( "?c=Purchases&a=Purchase", { id: purchase,status,title }).done(function( data ) {
            $('#xlModal').modal('toggle');
            $('#xlModal .modal-content').html(data);
        });
    });
});

$(document).on("click", ".rejections", function() {
    $("#loading").show();
    id = $(this).data('id');
    type = $(this).data('type');
    $.post( "?c=Init&a=Rejections", { id,type }).done(function( data ) {
        $("#loading").hide();
        $('#lgModal').modal('toggle');
        $('#lgModal .modal-content').html(data);
  });
});

$(document).on('submit', '#reject_form', function(e) {
  e.preventDefault();
  if (document.getElementById("reject_form").checkValidity()) {
      $("#loading").show();
      $.post( "?c=Init&a=Reject", $( "#reject_form" ).serialize()).done(function(res) {
        location.reload();
      });
  }
});


$(document).on("click", ".woAction", function() {
    id = $(this).data('id');
    status = $(this).data('status');
    title = $(this).data('title');
    if (status == 'view') {
        window.open("?c=WorkOrders&a=WorkOrder&status=view&id="+id);
    } else {
        $("#loading").show();
        $.post("?c=WorkOrders&a=WorkOrder",{id,status,title}).done(function(data){
            $("#loading").hide();
            $('#xlModal').modal('toggle');
            $('#xlModal .modal-content').html(data);
            $('.select2').select2(
                {
                tags:true,
            }
            );
            
        });
    }
});

$(document).on("click", ".bomAction", function() {
    id = $(this).data('id');
    status = $(this).data('status');
    title = $(this).data('title');
    if (status == 'view') {
        window.open("?c=BOM&a=BOM&status=view&id="+id);
    } else {
        $("#loading").show();
        $.post("?c=BOM&a=BOM",{id,status,title}).done(function(data){
            $("#loading").hide();
            $('#xlModal').modal('toggle');
            $('#xlModal .modal-content').html(data);
        });
    }
});

$(document).on("click", ".itAction", function() {
    id = $(this).data('id');
    status = $(this).data('status');
    title = $(this).data('title');
    $("#loading").show();
    $.post("?c=IT&a=ServiceDesk",{id,status,title}).done(function(data){
        $("#loading").hide();
        $('#xlModal').modal('toggle');
        $('#xlModal .modal-content').html(data);
        $('.select2').select2(
            {
            tags:true,
        }
        );
    });
});

$(document).on("click", ".mntAction", function() {
    id = $(this).data('id');
    status = $(this).data('status');
    title = $(this).data('title');
    $("#loading").show();
    $.post("?c=MNT&a=ServiceDesk",{id,status,title}).done(function(data){
        $("#loading").hide();
        $('#xlModal').modal('toggle');
        $('#xlModal .modal-content').html(data);
    });
});


$(document).on("click", ".reserveAction", function() {
    id = $(this).data('id');
    status = $(this).data('status');
    title = $(this).data('title');
    $("#loading").show();
    $.post("?c=Reserve&a=Reserve",{id,status,title}).done(function(data){
        $("#loading").hide();
        $('#xlModal').modal('toggle');
        $('#xlModal .modal-content').html(data);
        $('.select2').select2(
            {
            tags:true,
        }
        );
    });
});

$(document).on("click", ".checkService", function() {
    id = $(this).data('id');
    status = $(this).data('status');
    title = $(this).data('title');
    $("#loading").show();
    $.post( "?c=IT&a=Check", { id }).done(function( data ) {
        $.post("?c=IT&a=ServiceDesk",{id,status,title}).done(function(data){
            $("#loading").hide();
            $('#xlModal').modal('toggle');
            $('#xlModal .modal-content').html(data);
            $('.select2').select2(
                {
                tags:true,
            }
            );
        });
    });
});

$(document).on("click", ".checkmntService", function() {
    id = $(this).data('id');
    status = $(this).data('status');
    title = $(this).data('title');
    $("#loading").show();
    $.post( "?c=MNT&a=Check", { id }).done(function( data ) {
        $.post("?c=MNT&a=ServiceDesk",{id,status,title}).done(function(data){
            $("#loading").hide();
            $('#xlModal').modal('toggle');
            $('#xlModal .modal-content').html(data);
            $('.select2').select2(
                {
                tags:true,
            }
            );
        });
    });
});

$(document).on("click", ".checkReserveCancel", function() {
    id = $(this).data('id');
    $("#loading").show();
    $.post( "?c=Reserve&a=CheckCancel", { id }).done(function( data ) {
        $("#loading").hide();
    });
});

$(document).on("click", ".checkReserveApproved", function() {
    id = $(this).data('id');
    $("#loading").show();
    $.post( "?c=Reserve&a=Check", { id }).done(function( data ) {
        $("#loading").hide();
    });
});

if ( window.history.replaceState ) {
    window.history.replaceState( null, null, window.location.href );
}

$(document).on('select2:open', () => {
    document.querySelector('.select2-search__field').focus();
});

$(document).on('submit', '#purchase_form', function(e) {
  e.preventDefault();
  var errors = 0;
  status = $(this).data('status');
  if (status == 'process') {
    if ($(".copy").length == 0) {
      toastr.error('Add at least one item');   
      errors++;
    }
  }
  if (status == 'pricing') {
    var values = $(".vendorcheck").map(function(){return $(this).html().trim();}).get();
    n = values.includes('0'); 
    if (n)  {
        toastr.error('Add at least one Vendor for each Item');   
        errors++;
    }
  }
  if (status == 'receive') {
    var suma = 0;
    $('.partial').each(function() {
        suma += parseFloat($(this).html());
    });
    var sumaQty = 0;
    $('.itemQty').each(function() {
      sumaQty += parseFloat($(this).html());
    });
    if (suma < sumaQty) {
      toastr.error('All items must be complete'); 
      errors++;  
    }
  }
  if (errors == 0) {
    if (document.getElementById("purchase_form").checkValidity()) {
      $("#loading").show();
      var formData = new FormData(this);
      formData.append('status', status);
      $.ajax({
          url: "?c=Purchases&a=Update",
          type: 'POST',
          data: formData,
          success: function (data) {
            location.reload();
          },
          cache: false,
          contentType: false,
          processData: false
      });
    }
  }
});

$(document).on('submit', '#WO_form', function(e) {
    e.preventDefault();
    status = $(this).data('status');
    if (status == 'process') {
      if ($(".copy").length == 0) {
        toastr.error('Add at least one item');
        return;
      }
    }
    if (document.getElementById("WO_form").checkValidity()) {
      $("#loading").show();
      var formData = new FormData(this);
      formData.append('status', status);
      $.ajax({
        url: "?c=WorkOrders&a=FormSave",
        type: 'POST',
        data: formData,
        success: function (data) {
          location.reload();
        },
        cache: false,
        contentType: false,
        processData: false
      });
    }
});

$(document).on('submit', '#BOM_form', function(e) {
    e.preventDefault();
    status = $(this).data('status');
    if (status == 'process') {
        if ($(".copy").length == 0) {
            toastr.error('Add at least one item');
            return;
        }
    }
    if (status !== 'process') {
      var partial = 0;
      $('.partial').each(function() {
        partial += parseFloat($(this).html());
      });
      var qty = 0;
      $('.qty').each(function() {
        qty += parseFloat($(this).html());
      });
      if (qty != partial) {
            toastr.error('All items must be deliver before store');
            return;
      }
    }

    if (document.getElementById("BOM_form").checkValidity()) {
        $("#loading").show();
        var formData = new FormData(this);
        formData.append('status', status);
        $.ajax({
            url: "?c=BOM&a=FormSave",
            type: 'POST',
            data: formData,
            success: function (data) {
                location.reload();
            },
            cache: false,
            contentType: false,
            processData: false
        });
    }
});

$(document).on('submit', '.reserve_form', function(e) {
    e.preventDefault();
    status = $(this).data('status');
    url = '?c=Reserve&a=Save';
    if(status){url = '?c=Reserve&a=Update'}
    if(this.checkValidity()) {
        if ($("#total").html() == 0){
            toastr.error('Quantity is 0');
            return;
        }
        $("#loading").show();
        var formData = new FormData(this);
        formData.append('status', status);
        $.ajax({
            url: url,
            type: 'POST',
            data: formData,
            success: function (data) {
            location.reload();
            },
            cache: false,
            contentType: false,
            processData: false
      });
    }
});

$(document).on("click", ".purchaseCode", function() {
    id = $(this).data('id');
    $("#loading").show();
    $.post("?c=Purchases&a=Code",{id}).done(function(data){
        $("#loading").hide();
        $('#xsModal').modal('toggle');
        $('#xsModal .modal-content').html(data);
    });
});

$(document).on("click", ".purchaseCodeCheck", function() {
    $("#loading").show();
    id = $(this).data('id');
    status = 'view';
    title = 'View Purchase';
    $.post( "?c=Purchases&a=CodeCheck", { id }).done(function( data ) {
        location.reload();
    });
});
</script>